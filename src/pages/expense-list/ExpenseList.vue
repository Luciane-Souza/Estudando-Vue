<template>
  <div>
    ...
    <div class="container-group">
      <div class="container">
        <div v-if="activeMonth.data &&!activeMonth.data.length">Você não cadastrou nenhum gasto nesse mês.</div>
        <template v-else>
          <expense-list-item :key="index" :data="item" v-for="(item, index) in activeMonth.data" />
        </template>
      </div>
    </div>
  </div>
    ...
    groupedMonths() {
      ...
      if (this.expenses.length) {
        const months = groupBy(this.expenses, i =>
          moment(i.createAt).format("MM/YYYY")
        );
